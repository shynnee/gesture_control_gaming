...
class AnimatedSprite(Sprite):
	def __init__(self, pos, frames, groups, z = Z_LAYERS['main'], animation_speed = ANIMATION_SPEED):
		...
		super().__init__(pos, self.frames[self.frame_index], groups, z) if 0 <= self.frame_index < len(self.frames) else False
...
class MovingSprite(AnimatedSprite):
	def __init__(self, frames, groups, start_pos, end_pos, move_dir, speed,flip = False):
		...
		self.moving = True
		self.speed = speed
		self.direction = vt(1,0) if move_dir == 'x' else vt(0,1)
	...
	def update(self, dt):
		self.old_rect = self.rect.copy()
		self.rect.topleft += self.direction * self.speed*2 * dt
		self.check_border()
...
class Icon(pygame.sprite.Sprite):
	...
	def find_path(self):
		if self.path:
			if self.rect.centerx == self.path[0][0]:#thang dung
				self.direction = vt(0,1 if self.path[0][1] > self.rect.centery else -1)
			else: # ngang
				self.direction = vt(1 if self.path[0][0] > self.rect.centerx else -1,0)
		else:
			self.direction = vt()
	...
	def get_state(self):
		self.state = 'idle'
		if self.direction == vt(1,0):
			self.state = 'right'
		if self.direction == vt(-1,0):
			self.state = 'left'
		if self.direction == vt(0,1):
			self.state = 'down'
		if self.direction == vt(0,-1):
			self.state = 'up'
	def update(self,dt):
